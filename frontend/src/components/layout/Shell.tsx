...
function calcTimeLeft(expiresAt: string, sessionHours: number): TimeLeft {
  ...
  const pct = sessionMs > 0 ? Math.min(1, totalMs / sessionMs) : 0;
  return {
    totalMs,
    h, m, s, pct,
    urgent:  totalMs > 0 && totalMs < TEN_MINUTES,
    expired: totalMs === 0,
  };
}

function SessionTimerBanner({
  expiresAt,
  sessionHours,
}: {
  expiresAt: string;
  ...
  const dashOffset = tl.expired ? circumference : circumference * (1 - tl.pct);

  const ringColor = tl.expired
    ? 'text-white/10'
    : tl.urgent
      ? 'text-red-400'
      : tl.pct > 0.25
  ...
  const labelColor = tl.expired
    ? 'text-white/20'
    : tl.urgent
      ? 'text-red-400'
      : 'text-signal';

  const timeStr = tl.expired
    ? 'Expired'
    : (tl.h + ':' + fmt2(tl.m) + ':' + fmt2(tl.s));

  const subLabel = tl.expired
    ? 'Your session has ended'
    : tl.urgent
      ? 'Less than 10 min left'
      : (Math.ceil(tl.totalMs / 60000) + ' min remaining');

  const bannerBg = (tl.urgent && !tl.expired)
    ? 'bg-red-500/[0.07] border-red-500/20'
    : 'bg-signal/[0.07] border-signal/20';

  return (
    <div className={'mb-2 mx-1 animate-fade-in transition-colors duration-700 rounded-xl border px-4 py-2.5 ' + bannerBg}>
      <div className="flex items-center gap-3">

        {/* Arc progress ring */}
        <div className="relative shrink-0 w-10 h-10">
          <svg viewBox="0 0 100 100" className={'w-10 h-10 -rotate-90 transition-colors duration-700 ' + ringColor}>
            <circle cx="50" cy="50" r={R} fill="none"
              stroke="currentColor" strokeWidth="8" opacity={0.12} />
            <circle cx="50" cy="50" r={R} fill="none"
        ...
        <div className="flex-1 min-w-0">
          <div className={'font-mono text-[15px] font-bold tabular-nums leading-none mb-0.5 transition-colors duration-700 ' + labelColor}>
            {timeStr}
          </div>
          <div className="text-[9px] font-display font-bold uppercase tracking-[0.15em] text-white/30 leading-none">
            {subLabel}
          </div>
        </div>
        ...
        {/* Status badge */}
        <div className={
          'shrink-0 flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[9px] font-display font-bold uppercase tracking-widest border transition-colors duration-700 ' +
          (tl.expired
          ...
          {!tl.expired && (
            <span className={'w-1.5 h-1.5 rounded-full shrink-0 ' + (tl.urgent ? 'bg-red-400' : 'bg-signal animate-pulse-soft')} />
          )}
...
function ConnectionChip({ isOnline, urgent }: { isOnline: boolean; urgent?: boolean }) {
  if (isOnline) {
    const chipClass = urgent
      ? 'chip bg-red-500/10 border border-red-500/20 text-red-400'
      : 'chip chip-live';
    const dotClass = urgent
      ? 'w-1.5 h-1.5 rounded-full shrink-0 bg-red-400'
      : 'w-1.5 h-1.5 rounded-full shrink-0 bg-signal animate-pulse-soft';
    return (
...
export function Shell({ children }: { children: ReactNode }) {
  ...
  // status.accessGranted only means the portal DB granted — not that chilli
  // has actually moved the session to 'pass' yet, or that the session is still
  // active. Probing a real URL gives the ground truth.
  ...
  const urgent = !!(
    isOnline &&
    status?.expiresAt &&
    (new Date(status.expiresAt).getTime() - Date.now()) < TEN_MINUTES
  );
