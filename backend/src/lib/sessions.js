...
function getOrCreateSession(ip, campaignId, mac = null, dst = null, challenge = null, watchFrequency = 'once_per_day') {
  ...
  if (row) {
    ...
    if (needsNewSession(existing, watchFrequency)) {
      ...
      // once_per_day: NEVER carry forward. The new session always starts clean
      // (video_watched=0, survey_done=0) so the user re-watches on the next day.
      // Same-day expiry restriction ("come back tomorrow") is detected by the
      // /api/restrictions endpoint which queries historical granted sessions —
      // it does NOT rely on the current session's video_watched flag.
