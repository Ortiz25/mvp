...
function getCampaignBySlug(slug) {
  const db = getDb();
  const r = db.prepare('SELECT * FROM campaigns WHERE slug=?').get(slug);
  db.close();
  return r || null;
}
...
function getCampaignConfig(slug) {
  ...
  const v  = db.prepare(
    'SELECT * FROM campaign_videos WHERE campaign_id=? AND active=1 ORDER BY sort_order ASC LIMIT 1'
  ).get(c.id);
  const s  = db.prepare(
    'SELECT * FROM campaign_surveys WHERE campaign_id=? AND active=1 LIMIT 1'
  ).get(c.id);
  const qs = s
    ? db.prepare('SELECT * FROM survey_questions WHERE survey_id=? ORDER BY sort_order ASC')
        .all(s.id)
...
function createCampaign(d) {
  ...
  const id = uuidv4();
  db.prepare(
    'INSERT INTO campaigns(id,slug,name,description,sponsor,logo_url,primary_color,accent_color,bg_color,session_hours,start_date,end_date,watch_frequency,require_survey,require_video) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)'
  ).run(
    ...
    d.require_video  !== undefined ? (d.require_video  ? 1 : 0) : 1
  );
  const r = db.prepare('SELECT * FROM campaigns WHERE id=?').get(id);
  db.close();
  return r;
...
function updateCampaign(id, d) {
  ...
  const allowed = [
    ...
    'watch_frequency', 'require_survey', 'require_video',
  ];
  const fields = [], vals = [];
  ...
  if (fields.length) {
    ...
    vals.push(id);
    db.prepare(`UPDATE campaigns SET ${fields.join(',')} WHERE id=?`).run(...vals);
  }
  const r = db.prepare('SELECT * FROM campaigns WHERE id=?').get(id);
  db.close();
  return r;
...
function getVideosForCampaign(cid) {
  const db = getDb();
  const r = db.prepare('SELECT * FROM campaign_videos WHERE campaign_id=? ORDER BY sort_order ASC').all(cid);
  db.close();
  return r;
}
...
function createVideo(cid, d) {
  ...
  const id = uuidv4();
  db.prepare(
    'INSERT INTO campaign_videos(id,campaign_id,title,description,filename,thumbnail_filename,duration_seconds,required_watch_pct,sort_order) VALUES(?,?,?,?,?,?,?,?,?)'
  ).run(
    id, cid, d.title, d.description || null, d.filename,
    d.thumbnail_filename || null, d.duration_seconds || 120,
    d.required_watch_pct || 0.8, d.sort_order || 0
  );
  const r = db.prepare('SELECT * FROM campaign_videos WHERE id=?').get(id);
  db.close();
  return r;
}
...
function updateVideo(id, d) {
  const db = getDb();
  const allowed = [
    'title', 'description', 'filename', 'thumbnail_filename',
    'duration_seconds', 'required_watch_pct', 'sort_order', 'active',
  ];
  const fields = [], vals = [];
  for (const k of allowed) {
    if (d[k] !== undefined) { fields.push(`${k}=?`); vals.push(d[k]); }
  ...
  if (fields.length) {
    vals.push(id);
    db.prepare(`UPDATE campaign_videos SET ${fields.join(',')} WHERE id=?`).run(...vals);
  }
  const r = db.prepare('SELECT * FROM campaign_videos WHERE id=?').get(id);
  db.close();
  return r;
...
function deleteVideo(id) {
  const db = getDb();
  db.prepare('DELETE FROM campaign_videos WHERE id=?').run(id);
  db.close();
}
...
function getSurveyForCampaign(cid) {
  ...
  const qs = db.prepare('SELECT * FROM survey_questions WHERE survey_id=? ORDER BY sort_order ASC')
    .all(s.id)
    .map(q => ({ ...q, options: JSON.parse(q.options) }));
  db.close();
...
function upsertSurvey(cid, d) {
  ...
  db.transaction(() => {
    ...
    if (!s) {
      const sid = uuidv4();
      db.prepare('INSERT INTO campaign_surveys(id,campaign_id,title) VALUES(?,?,?)').run(sid, cid, d.title || 'Survey');
      s = db.prepare('SELECT * FROM campaign_surveys WHERE id=?').get(sid);
    } else {
      db.prepare('UPDATE campaign_surveys SET title=? WHERE id=?').run(d.title || s.title, s.id);
    }
  ...
  db.close();
  return getSurveyForCampaign(cid);
}

module.exports = {
  getAllCampaigns, getCampaignBySlug, getCampaignById, getCampaignConfig,
  createCampaign, updateCampaign, getVideosForCampaign, createVideo,
  updateVideo, deleteVideo, getSurveyForCampaign, upsertSurvey,
};