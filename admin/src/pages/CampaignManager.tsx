    ...
    <div className="p-6">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          {campaigns.map(c => (
            <CampaignCard// src/pages/CampaignManager.tsx
            import { useEffect, useRef, useState } from 'react';
            import { api, Campaign, Video, Survey } from '../lib/api';
            ...
            const FREQ_LABEL: Record<string, { label: string; icon: string; desc: string }> = {
              once_per_day: { label: 'Once / day',    icon: '📅', desc: 'Watch once daily — same-day top-ups skip video' },
              once_ever:    { label: 'Once ever',     icon: '♾️',  desc: 'Watch once per campaign — never required again' },
              always:       { label: 'Every session', icon: '🔁', desc: 'Must watch on every single grant' },
            };
            ...
            function CampaignCard({
              ...
              const on   = c.active === 1;
              const freq = FREQ_LABEL[c.watch_frequency] ?? FREQ_LABEL['once_per_day'];
